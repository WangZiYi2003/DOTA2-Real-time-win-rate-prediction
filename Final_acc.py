...
def main():
    ...
    for interval in time_intervals:
        avg_acc1 = sum(accuracies1[interval]) / len(accuracies1[interval]) if len(accuracies1[interval]) > 0 else 0
        avg_acc2 = sum(accuracies2[interval]) / len(accuracies2[interval]) if len(accuracies2[interval]) > 0 else 0
        avg_acc3 = sum(accuracies3[interval]) / len(accuracies3[interval]) if len(accuracies3[interval]) > 0 else 0
        avg_combined_acc = sum(combined_accuracies[interval]) / len(combined_accuracies[interval]) if len(
            combined_accuracies[interval]) > 0 else 0

        print(f'Time interval: {interval[0]} - {interval[1]}')
        print(f'Model 1 Average Accuracy: {avg_acc1:.2f}')
        print(f'Model 2 Average Accuracy: {avg_acc2:.2f}')
        print(f'Model 3 Average Accuracy: {avg_acc3:.2f}')
        print(f'Combined Model Average Accuracy: {avg_combined_acc:.2f}')
        print()
